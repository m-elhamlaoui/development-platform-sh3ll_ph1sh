...
const FilesPage = () => {
  ...
  const loadFiles = async (subjectName) => {
    ...
    try {
      ...
      setFiles(fileData);
      // Add this log to inspect file data, including fileType
      console.log('Files loaded for subject:', subjectName, fileData);
    } catch (error) {
  ...
  const filteredFiles = files.filter(file => {
    ...
    const matchesFileType =
      fileType === 'All' || (file.fileType && file.fileType.toUpperCase() === fileType.toUpperCase());
    ...
    return filteredFiles.map(file => (
      ...
      >
        <div className="file-icon">
          {file.fileType === 'PDF' ? <FileOutlined /> :
           file.fileType === 'Video' ? <FaVideo /> :
           file.fileType === 'Audio' ? <FaMusic /> :
           <FileOutlined />}
        </div>
        <div className="file-info">
          <h3 className="file-title">{file.title}</h3>
          <p className="file-meta">
            {file.fileType} • {new Date(file.created_at).toLocaleDateString()}
          </p>
        </div>
        <div className="file-actions">
          ...
          >
            {file.is_favorite ? <StarFilled /> : <StarOutlined />}
          </button>
          <button
          ...
          {/* Show delete button if user is admin OR the file owner */}
